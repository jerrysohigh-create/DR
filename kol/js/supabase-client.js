...
async function call(name,body,token){
  const r=await fetch(`${cfg.SUPABASE_URL}/functions/v1/${name}`,{method:'POST',headers:{'content-type':'application/json',apikey:cfg.SUPABASE_ANON_KEY,authorization:`Bearer ${cfg.SUPABASE_ANON_KEY}`},body:JSON.stringify({...body,token}),signal:AbortSignal.timeout(15000)});
